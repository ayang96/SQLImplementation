...
public class BNLJOperator extends JoinOperator {
  ...
  public int estimateIOCost() throws QueryPlanException {
    /* TODO: Implement me! */
        ...
        /*  it's ceil(R/(B-2)) * S + R not ceil(R/(B-2) *S + R)*/
    int leftNumPages=this.getLeftSource().getStats().getNumPages();
    int rightNumPages=this.getRightSource().getStats().getNumPages();
    double bufferSize=this.numBuffers-2;
    double estimate=(double) Math.ceil(leftNumPages/bufferSize)* ( double) rightNumPages+leftNumPages;
    return (int) estimate;
  }
