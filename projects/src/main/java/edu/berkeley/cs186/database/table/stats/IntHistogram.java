...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    int predValue = value.getInt();
    int totalCount = 0;
    int totalOccurrence = 0; //average occurrence of targeted value
    ...
    for (Bucket<Integer> bucket : buckets) {
      ...
      int bucketOccurrence = bucketCount / bucketSize;

      if (!foundone || maxValue < bucketEnd-1) {
        maxValue = bucketEnd-1; //the bucket end is exclusive;
      }
      if (!foundone || minValue > bucketStart) {
      ...
      switch (predicate) {
        ...
        case NOT_EQUALS:
          if (predValue >= bucketStart && predValue < bucketEnd) {
            totalOccurrence += bucketOccurrence;
          }
      }
    }

    switch (predicate) {
        case EQUALS: //Use the bucket-based formula
          return ((float) totalOccurrence / (float) totalCount);
        case NOT_EQUALS:
          return (1 - (float)totalOccurrence / (float) totalCount);
        case GREATER_THAN:

          return ((float) (maxValue+1 - predValue) / (float) (maxValue - minValue + 1));
        case LESS_THAN_EQUALS://opposite to >
          //return (1 - (float) (maxValue - predValue) / (float)(maxValue - minValue + 1));
          return (float) ((float)(predValue-minValue )/(float)(maxValue - minValue +1)
                  +1.0/ (float)this.getNumDistinct());
        case GREATER_THAN_EQUALS:
          //maxValue++;
          return (float) ((float)(maxValue+1 - predValue )/(float)(maxValue - minValue +1)
                            +1.0/ (float)this.getNumDistinct());
        case LESS_THAN:
          return ( (float)(predValue-minValue) / (float)(maxValue - minValue + 1));
      }
