...
public class TestOptimalQueryPlan {
  ...
  @Test
  @Category(StudentTestP4.class)
  public void testSimpleSelectIterator1() throws DatabaseException, QueryPlanException {
    Database.Transaction transaction = this.database.beginTransaction();
    QueryPlan queryPlan = transaction.query(this.defaulTableName);

    queryPlan.select("int", QueryPlan.PredicateOperator.NOT_EQUALS, new IntDataBox(5));
    ...
    while (outputIterator.hasNext()) {
      ...
      assertTrue(record.getValues().get(1).getInt() != 5);
    }
